...
class QueryBuilder
{
    ...
    /**
     * Modifies the LIMIT clause based on limit and offset
     *
     * @param $query
     * @param $limit
     * @param null $offset
     ...
     * @throws \Exception
     */
    protected function modifyLimitQuery($query, $limit, $offset = null)
    {
        if ($limit !== null) {
            $limit = (int) $limit;
        }
    ...
    private function getSQLForUpdate()
    {
        ...
        # Skip alias if it is the same as the table name
        if($this->sqlParts['from']['alias'] &&  $this->sqlParts['from']['alias'] !== $this->sqlParts['from']['table']) {
            $table .= ' ' . $this->sqlParts['from']['alias'];
        }
